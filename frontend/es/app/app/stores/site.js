...
export const useSitesStore = defineStore(
  ...
  () => {
    ...
    // ✅ SOLO ESTO SE PERSISTE (Ahora gestionado por el plugin)
    ...
    function setLocation(newLocation) {
      location.value = {
        ...location.value,
        ...newLocation,
        neigborhood: newLocation?.neigborhood
          ? newLocation.neigborhood
          : emptyNeighborhood(),
      }
    ...
    function updateLocation(data, price = 0) {
      ...
      if (data?.address_details !== undefined)
        location.value.address_details = data.address_details
      if (data?.formatted_address !== undefined)
        location.value.formatted_address = data.formatted_address || ''
